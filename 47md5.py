...
constants = [int(abs(math.sin(i + 1)) * 4294967296) & 0xFFFFFFFF for i in range(48)]
...
def processMessage(msg):
    ...
    for offset in range(0, len(msg), 64):
        ...
        # Round 3: 15 steps (until step 47)
        for i in range(32, 47):
            ...
            to_rotate = A + F + constants[i % 16] + int.from_bytes(block[4 * G:4 * G + 4], byteorder='little')
            newB = (B + leftRotate(to_rotate, rotate_by[i % 16])) & 0xFFFFFFFF
...
def md5_47_step(msg):
    ...
    print("47-Step MD5 Hash: ", message_hash)
